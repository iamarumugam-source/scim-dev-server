...
import {
  ...
  CalendarDays, User, Video,
  ArrowLeft, ArrowRight, X, GripVertical,
} from "lucide-react";
...
import {
  CELL_W, GRID_W, ROW_H, HEAD_H,
  avatarColor, initials, fmt, getLocalHour, blockStyle,
  toLocalISODate, parseDateStr, formatDisplayDate, buildRefDate,
  formatHour, addDays, durationLabel,
} from "./utils";
...
export default function TimeConverterPage() {
  ...
  const startDrag = useCallback((which: "start" | "end", startClientX: number) => {
    ...
    if (!el) return;
    const baseLocalMins = toLocalPos(which === "start" ? startMinutes : endMinutes);
    setDragging(which);
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="flex items-center border border-border rounded-md overflow-hidden">
              {[5, 10, 15, 30].map((m) => (
                <button key={m} onClick={() => setSnapMins(m)}
                  className={cn("px-2 h-8 text-xs font-medium transition-colors",
          ...
          {/* 12h / 24h toggle */}
          <div className="flex items-center border border-border rounded-md overflow-hidden">
            <button onClick={() => setUse24h(true)}
              className={cn("px-2.5 h-8 text-xs font-medium transition-colors",
                use24h ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted")}>
              24h
            </button>
            <button onClick={() => setUse24h(false)}
              className={cn("px-2.5 h-8 text-xs font-medium transition-colors",
                !use24h ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted")}>
              12h
            </button>
          ...
          <Popover>
            ...
            <PopoverContent className="w-auto p-0" align="end">
              <Calendar mode="single" selected={parseDateStr(dateStr)}
      ...
      {allZones.length > 0 && (
        <div className="border border-border rounded-lg overflow-hidden bg-card">
          ...
          <div className="flex items-center justify-between border-b border-border bg-muted/30 px-4 py-1.5">
            ...
            <div className="flex items-stretch rounded-full overflow-hidden border border-primary/40 shadow-sm">
              <button
                onClick={() => setDateStr(addDays(dateStr, -1))}
                className="px-3 h-7 flex items-center text-primary bg-primary/5 hover:bg-primary hover:text-primary-foreground transition-colors border-r border-primary/30"
              >
                <ArrowLeft className="h-3.5 w-3.5" />
              </button>
              <button
                onClick={() => setDateStr(addDays(dateStr, 1))}
                className="px-3 h-7 flex items-center text-primary bg-primary/5 hover:bg-primary hover:text-primary-foreground transition-colors"
              >
          ...
          <div className="flex">
            ...
            <div className="flex-shrink-0 w-52 border-r border-border">
              ...
              {allZones.map((tz, allIdx) => {
                ...
                const isDropTarget = !isUserTz && dragOver === zonesIdx && dragFrom !== zonesIdx;
                return (
                  <div key={tz.id}
                    draggable={!isUserTz}
                    onDragStart={!isUserTz ? () => setDragFrom(zonesIdx) : undefined}
                    onDragOver={!isUserTz ? (e) => { e.preventDefault(); setDragOver(zonesIdx); } : undefined}
                    onDrop={!isUserTz ? () => {
                    ...
                    className={cn(
                      ...
                      isUserTz ? "bg-primary/[0.03]" : "cursor-grab active:cursor-grabbing",
                    ...
                    <div className="min-w-0 flex-1 overflow-hidden">
                      <div className="flex items-center gap-1 leading-none">
                        ...
                        {isUserTz && <span className="text-[9px] bg-primary/10 text-primary px-1 rounded font-medium flex-shrink-0">You</span>}
                      </div>
                      <div className="flex items-center gap-1 mt-0.5">
                        {tz.country && <span className="text-[10px] text-muted-foreground font-medium">{tz.country}</span>}
                    ...
                    {!isUserTz && (
                      <Button variant="ghost" size="icon" className="h-5 w-5 text-muted-foreground flex-shrink-0 -mr-1"
                        onClick={(e) => { e.stopPropagation(); setZones((p) => p.filter((z) => z.id !== tz.id)); }}>
                        <X className="h-3 w-3" />
                      </Button>
                    )}
                  </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-x-auto scrollbar-hide min-w-0">
              <div style={{ width: GRID_W }} className="relative">
                ...
                <div style={{ height: HEAD_H }}
                  ...
                  {Array.from({ length: 24 }, (_, localH) => (
                    <div key={localH} style={{ width: CELL_W, height: HEAD_H }}
                      className="flex-shrink-0 flex items-end justify-center pb-2 text-[11px] font-mono font-medium pointer-events-none text-muted-foreground/50">
                ...
                <div className="absolute top-0 bottom-0 bg-primary/15 pointer-events-none"
                ...
                <Cursor minutes={startMinutes} label="Start" variant="start"
                  ...
                  onDragStart={(e) => { e.stopPropagation(); startDrag("start", "touches" in e ? e.touches[0].clientX : e.clientX); }} />
                <Cursor minutes={endMinutes} label="End" variant="end"
                  isActive={dragging === "end"} gridW={GRID_W}
                  dateStr={dateStr} userTz={userTz} userTzCity={userTzInfo.city}
                  use24h={use24h} userOffsetMins={userOffsetMins}
                  onDragStart={(e) => { e.stopPropagation(); startDrag("end", "touches" in e ? e.touches[0].clientX : e.clientX); }} />
            ...
            {allZones.map((tz) => {
              ...
              return (
                <div key={tz.id} className="flex items-center gap-1.5 flex-shrink-0">
                  ...
                  {day1 !== dayBase && <span className="text-[10px] text-muted-foreground">({day1})</span>}
                </div>
              );
            })}
