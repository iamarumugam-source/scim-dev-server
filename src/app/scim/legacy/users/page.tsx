...
// ─── ARCHIVED ─────────────────────────────────────────────────────────────────
//
// The previous users page, superseded by /scim/users. Kept as an unlinked route so it stays
// clickable for side-by-side comparison and quick rollback — it is not in the
// sidebar `items` array, but it IS under /scim/* so the auth middleware still
// covers it.
//
// Nothing imports this file. To roll back, move it over /scim/users (its git history
// is intact under the old path).
//
// Not actively maintained: it reads the same /stats endpoint, so response shape
// changes can break it without warning.

import { useEffect, useState, useMemo, useCallback } from "react";
import { ScimUser } from "@/lib/scim/models/scimSchemas";
import { useSession } from "next-auth/react";
import { motion, AnimatePresence } from "motion/react";
import { getColumns, UserExpandedRow } from "../../users/columns";
...
export default function UsersPage() {
  ...
  return (
    ...
    >
      {/* Archived — see the header comment. */}
      <div className="flex items-start gap-2.5 rounded-lg border border-dashed px-3.5 py-2.5 text-xs text-muted-foreground">
        <Archive className="mt-0.5 h-3.5 w-3.5 flex-shrink-0" />
        <span>
          <strong className="text-foreground">Archived users page.</strong> Kept for
          comparison and rollback only — not linked from navigation. The current
          users page lives at <code className="font-mono">/scim/users</code>.
        </span>
      </div>
